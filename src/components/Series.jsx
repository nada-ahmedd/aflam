...
const Series = () => {
  ...
  return (
    <div style={{ backgroundColor: '#000', minHeight: '100vh', color: '#fff', padding: '40px 20px' }}>
      <Navbar />
      <h1 className="text-center" style={{ color: '#00bcd4', fontSize: '3rem', marginBottom: '20px', textShadow: '0 0 10px #00bcd4', transition: 'all 0.3s' }}>Series</h1>
      ...
      <div style={{ width: '90%', margin: '0 auto' }}>
            ...
            <div key={item.id} className="col-md-3 col-sm-6 mb-4">
              <div className="card text-white" style={{ borderRadius: '15px', backgroundColor: '#1a1a1a', boxShadow: '0 6px 12px rgba(0, 0, 0, 0.5)', maxWidth: '250px', width: '100%', margin: '0 auto', transition: 'transform 0.3s' }}
                ...
                <div className="card-body" style={{ backgroundColor: '#1a1a1a', padding: '15px' }}>
                  <p style={{ fontSize: '1rem', margin: '0 0 10px', color: '#fff' }}><strong>{item.name}</strong></p>
                  ...
                  <Link to={`/details/tv/${item.id}`} className="btn btn-info w-100"
        ...
        {series.length === 0 && <p className="text-center" style={{ fontSize: '1.2rem', color: '#ccc' }}>No series available</p>}
      </div>
    </div>
  );
};
